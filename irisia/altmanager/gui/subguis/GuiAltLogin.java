...
import irisia.altmanager.gui.other.GuiPasswordField;
...
public class GuiAltLogin extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(this.mc.fontRendererObj, StringUtil.simpleTranslateColors((this.loginThread == null) ? "&eWaiting for login..." : this.loginThread.getStatus()), this.width / 2, 30, -1);
		...
		case 0:
			if (!this.email.getText().isEmpty()) {
				if (this.email.getText().contains(":")) {
					...
					Alt account1 = new Alt(split[0], split[1], split[0]);
					this.loginThread = new AccountLoginThread(account1.getEmail(), account1.getPassword());
					this.loginThread.start();
				}
				Alt account = new Alt(this.email.getText(), this.password.getText(), this.email.getText());
