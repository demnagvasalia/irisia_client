...
/*
    Created by someone (probably by Zed or Valkyrie)
    Modified by me
 */
public final class FontRenderer {
    ...
    public FontRenderer(final Font font, final int characterCount, final boolean antiAlias) {
        this.font = font;
    ...
    private CharacterData[] setup(final CharacterData[] characterData, final int type) {
        ...
        // Set the font to what we imported before
        ...
        // Setup metrics for all characters
    ...
    public void drawCenteredString(final String text, final float x, final float y2, final int color) {
        ...
        this.renderString(text, x - width, y2, color, true);
        GL11.glTranslated(-0.5, -0.5, 0.0);
    ...
    private void renderString(final String text, float x, float y2, final int color, final boolean shadow) {
        Gui.drawRect(0,0,0,0,0); // we do this because there is a bug with the renderer that forgets to enable blending.
