...
public class ShaderUtil {
    ...
    public ShaderUtil(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "shader/vertex.vert");
    }
    ...
    public static void drawQuads(float x, float y, float width, float height) {
        if (mce.gameSettings.ofFastRender) return;
        glBegin(GL_QUADS);
        glTexCoord2f(0, 0);
        glVertex2f(x, y);
        glTexCoord2f(0, 1);
    ...
    public static void drawQuads2(float x, float y, float width, float height) {
        if (mce.gameSettings.ofFastRender) return;
        glBegin(GL_QUADS);
        glTexCoord2f(0, 0);
        glVertex2f(x, y);
    ...
    public static void drawQuads() {
        if (mce.gameSettings.ofFastRender) return;
        ScaledResolution sr = new ScaledResolution(mce);
