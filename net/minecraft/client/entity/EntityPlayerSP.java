...
import irisia.event.system.impl.EventMessage;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    private EventMotion currentEvent;
    ...
    public float renderYaw,lastRenderYaw;
    ...
    public void onUpdate()
    {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            if ((mc.thePlayer.isBlocking() && mc.gameSettings.keyBindPickBlock.isPressed()) || (mc.thePlayer.isEating() && mc.gameSettings.keyBindPickBlock.isPressed())) {
                mc.clickMouse();
            }

            this.lastRenderYaw = this.renderYaw;
            this.renderYaw = mc.thePlayer.getRotationYawHead();
            super.onUpdate();
            ...
            else
            {
                ...
                currentEvent.setState(Event.State.POST);
                Irisia.getInstance.getInstanceCollection().getBus().publish(currentEvent);
            }
        }
    }
    ...
    public void moveEntity(double x, double y, double z){
        ...
        Irisia.getInstance.instanceCollection.getBus().publish(eventMove);
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        Irisia.getInstance.instanceCollection.getBus().publish(eventUpdate);
        ...
        final EventMotion eventMotion = new EventMotion(onGround, posX, posY, posZ, rotationYaw, rotationPitch);
        eventMotion.setState(Event.State.PRE);
        Irisia.getInstance.getInstanceCollection().getBus().publish(eventMotion);
        this.currentEvent = eventMotion;

        if (this.isCurrentViewEntity())
        {
            double x = eventMotion.getX();
            double y = eventMotion.getY();
            double z = eventMotion.getZ();
            boolean ground = eventMotion.isOnGround();
    ...
    {
        EventMessage em = new EventMessage(message);
        Irisia.getInstance.getInstanceCollection().getBus().publish(em);
        if(!em.isCancelled()) this.sendQueue.addToSendQueue(new C01PacketChatMessage(em.getContent()));
    }
