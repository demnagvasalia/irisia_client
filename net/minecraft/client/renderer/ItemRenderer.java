...
import irisia.module.impl.render.ViewModel;
...
public class ItemRenderer
{
    ...
    private void doItemUsedTransformations(float swingProgress)
    {
        ...
        float f1 = 0.2F * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI * 2.0F);
        float f2 = -0.2F * MathHelper.sin(swingProgress * (float)Math.PI);
        Animations animations = (Animations) Irisia.getInstance.getInstanceCollection().getModuleManager().getModuleByName("Animations");
        if(animations.isToggled()){
            if(animations.smoothHitProperty.isEnabled()){
                GlStateManager.rotate(30.0f, 0.0f, 0.0f, 0.0f);
                GlStateManager.rotate(30.0F, 0.0F, 0.0F, 0.0F);
            } else{
                GlStateManager.translate(f, f1, f2);
            }
    ...
    {
        Animations animations = (Animations) Irisia.getInstance.getInstanceCollection().getModuleManager().getModuleByName("Block Animations");
        ViewModel viewModel = (ViewModel) Irisia.getInstance.getInstanceCollection().getModuleManager().getModuleByName("View Model");

        if (viewModel.isToggled()) GlStateManager.translate(viewModel.xProperty.getValueFloat(), viewModel.yProperty.getValue(), viewModel.zProperty.getValue());
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            KillAura aura = (KillAura) Irisia.getInstance.getInstanceCollection().getModuleManager().getModuleByName("Kill Aura");
            Animations animations = (Animations) Irisia.getInstance.getInstanceCollection().getModuleManager().getModuleByName("Animations");
            if (this.itemToRender != null)
            {
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0 || (aura.autoBlockModeProperty.is("Fake") && aura.autoBlockProperty.isEnabled() && aura.isToggled() && aura.optimalTarget != null))
                {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            float modifier = MathHelper.sin(MathHelper.sqrt_float(f1) * 3.1415927F);

                            final float var95 = MathHelper.sin(MathHelper.sqrt_float(f1) * MathHelper.PI);
                            if(animations.isToggled()) {
                                switch (animations.animationsModeProperty.getSelected()) {
                                    case "1.7": {
                                    ...
                                    case "Exhibition": {
                                        ...
                                        GlStateManager.rotate(-var95 * 40.0F / 2.0F, var95 / 2.0F, -0.0F, 9.0F);
                                        GlStateManager.rotate(-var95 * 60.0F, 1.0F, var95 / 2.0F, -0.0F);
                                        this.func_178103_d();
                                        break;
                                    }
                                    case "Fan": {
                                    ...
                                    case "Swing": {
                                        ...
                                        GL11.glRotatef(-modifier * -50.0F / 2.0F, -8.0F, 0.0F, 9.0F);
                                        GL11.glRotatef(-modifier * 20.0F, 1.0F, -0.4F, -0.0F);
                                        break;
                                    }
                                    case "Swang": {
                                        this.transformFirstPersonItem(f, f1);
                                        GlStateManager.translate(0, 0.45f, 0);
                                        GlStateManager.rotate(-var95 * (float) 80.0 / 2.0f, -8.0f, 0.4f, 9.0f);
                                        GlStateManager.rotate(-var95 * (float) 45, 1.5f, -0.4f, -0.0f);
                                        break;
                                    }
                                    case "Swong": {
                                        ...
                                        var2 /= 1.0F;
                                        float var3 = (float) ((int) ((double) System.currentTimeMillis() / 3.5D % 120.0D));
